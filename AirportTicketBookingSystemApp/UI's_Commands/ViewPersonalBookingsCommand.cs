...
namespace AirportTicketBookingSystemApp.Commands_UI
{
    public class ViewPersonalBookingsCommand : IMenuCommands
    {
        ...
        private void printBookings(List<FlightBookingModel> bookings)
        {
            ...
            sb.AppendLine(" Booking Number                                      Class        Price             From         To          On");
            sb.AppendLine("----------------                                     ------       ------            ------       ------      ------");
            foreach (var record in bookings)
            {
                var flight = _systemFlights.Find(f => f.Number == record.FlightNumber);
                sb.AppendLine($"{record.BookingNumber,-54}{record.FlightClass,-13}{record.Price} {record.Currency,-13}{flight?.DepartureCountry,-13}{flight?.ArrivalCountry,-9}{flight?.DepartureDate.Date,-9}");
            }
            sb.ToString();
